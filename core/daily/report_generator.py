...
class DailyReportGenerator:
        ...
        try:
            ...
            if index_files:
                ...
                if all_returns:
                    ...
                    # 修复：volume的单位是股，不是手，所以不需要乘以100
                    # 只需要除以1000000000000转换为万亿
                    market_volume = float(total_amount) / 1000000000000
                    ...
                    if not market_df.empty:
                        ...
                        if not day_data.empty:
                            ...
                            if 'total_volume' in day_data.columns:
                                # 修复：市场总成交额应该除以1000000000000转换为万亿
                                market_volume = float(day_data['total_volume'].iloc[-1]) / 1000000000000
            except Exception:
                pass
    ...
    def _render_report(
        ...
        lines.append(f"- 市场成交量: {market.market_volume:.2f}万亿")
        # 修复：如果北向资金数据不可用，显示"数据不可用"
        if market.northbound_flow == 0:
            lines.append("- 北向资金净流入: 数据不可用")
        else:
        ...
        factor_ic = factors.get("factor_ic", {})
        factor_contribution = factors.get("factor_contribution", {})
        
        # 如果factor_ic和factor_contribution都是空的，显示alpha_factor_ids和alpha_factor_weights
        if not factor_ic or (len(factor_ic) == 1 and "无有效数据" in factor_ic):
            # 尝试从pipeline_data中获取alpha_factor_ids和alpha_factor_weights
            try:
                from pathlib import Path
                import json
                ...
                if pipeline_data_path.exists():
                    ...
                    alpha_factor_ids = pipeline_data.get("alpha_factor_ids", [])
                    alpha_factor_weights = pipeline_data.get("alpha_factor_weights", [])
                    
                    if alpha_factor_ids and alpha_factor_weights:
                        lines.append("### 当前使用的因子")
                        lines.append("")
                        for factor_id, weight in zip(alpha_factor_ids, alpha_factor_weights):
                            lines.append(f"- {factor_id}: 权重 {weight:.2%}")
                        lines.append("")
            except Exception:
                pass
